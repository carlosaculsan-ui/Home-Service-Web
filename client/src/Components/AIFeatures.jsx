          ...
          <div className="bg-gray-50 rounded-2xl p-8 text-center border border-gray-200">
            ...
            <div className="flex gap-3 flex-wrap justify-center mb-6">
              {["What services do you offer?", "How much does cleaning cost?", "Book a plumber"].map((suggestion, i) => (
                <span key={i} className="bg-orange-100 text-orange-600 text-sm px-4 py-2 rounded-full cursor-pointer hover:bg-orange-200">
                  {suggestion}
                </span>
              ))}
            </div>
          ...
          <div className="bg-gray-50 rounded-2xl p-8 border border-gray-200">
            ...
            <label className="block cursor-pointer">
              <div className={`border-2 border-dashed rounded-xl p-8 text-center transition-all ${
                imagePreview ? 'border-orange-400 bg-orange-50' : 'border-gray-300 hover:border-orange-400'
              }`}>
                {imagePreview ? (
            ...
            {imagePreview && !damageResult && (
              <button
                ...
                disabled={analyzing}
                className="w-full mt-4 bg-orange-500 text-white py-3 rounded-xl hover:bg-orange-600 font-semibold disabled:opacity-60"
              >
            ...
            {damageResult && (
              <div className="mt-6 bg-white rounded-xl border border-gray-200 p-6 space-y-3">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-orange-600 font-semibold text-sm">{damageResult.confidence}</span>
                </div>

                <div className="border-t pt-3">
                  <p className="text-gray-600 text-sm mb-2">{damageResult.description}</p>
                  <p className="text-orange-600 text-sm font-medium">{damageResult.recommendation}</p>
                </div>
